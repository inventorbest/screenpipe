...
import { GitContextPanel } from "./git-context-panel";
...
const AGENTS: Agent[] = [
  ...
  },
  {
    id: "git-sage",
    name: "git sage",
    description: "analyzes git commit history across repositories",
    systemPrompt:
      "you analyze git commit history from multiple repositories. help users understand code changes, development patterns, what work was done, when, and by whom. you can correlate git commits with screen activity for comprehensive insights.",
    dataSelector: (results) => results,
  },
];
...
export function SearchChat() {
  ...
  const { getPreset } = usePipeSettings("searchWithGit");
  ...
  const calculateSelectedContentLength = () => {
    let total = Array.from(selectedResults).reduce((sum, index) => {
      const item = results[index];
      if (!item || !item.type) return sum; // Add this check
  ...
  const handleSearch = async (newOffset = 0, overrides: any = {}) => {
    ...
    await pipe.captureMainFeatureEvent("searchWithGit", {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto p-4 mt-12">
      ...
      {/* Git Context Panel Toggle */}
      <div className="flex justify-center mb-4">
        <Button
          variant={showGitPanel ? "default" : "outline"}
          onClick={() => setShowGitPanel(!showGitPanel)}
          className="gap-2"
        >
          <GitBranch className="h-4 w-4" />
          {showGitPanel ? "hide git context" : "add git context"}
          {selectedGitCommits.size > 0 && (
            <Badge variant="secondary" className="ml-1">
              {selectedGitCommits.size}
            </Badge>
          )}
        </Button>
      </div>
      ...
      <div className="w-1/3 mx-auto">
        <AIPresetsSelector pipeName={"searchWithGit"} />
      </div>
